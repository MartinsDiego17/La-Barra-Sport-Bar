...
const crearProducto = async ({ image, name, category, price, stock, ingredients }) => {
    ...
    //VALIDACIONES
    {
        ...
        // IMAGEN
        if (!image) return "Debes indicar una imagen para el producto.";
        if (typeof image !== "string") return "Debes indicar una imagen válida.";

        // NOMBRE
        if (!name) return "Debes indicar un nombre para el producto.";
        if (typeof name !== "string") return "Debes indicar un nombre válido.";

        // CATEGORÍA
        if (!category) return "Debes indicar una categoría para el producto.";
        if (typeof category !== "string") return "Debes indicar una categoría válida.";

        // PRECIO
        if (!price) return "Debes indicar un precio para el producto.";
        if (typeof price !== "number") return "Debes indicar un precio válido.";

        // STOCK
        if (typeof stock !== "boolean") return "Debes indicar un stock válido.";
